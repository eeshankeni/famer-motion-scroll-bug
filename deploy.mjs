...
const baseurl = process.env.STORAGEZONE_BASE_URL; // contains trailing slash
const apikey = process.env.STORAGEZONE_API_KEY;
const pullzone = process.env.PULLZONE_BASE_URL; // contains trailing slash
const accountkey = process.env.ACCOUNT_KEY;

if (!(baseurl && apikey && pullzone && accountkey)) {
  throw new Error(
...
const deploy = async () => {
  ...
  try {
    const files = getFiles("./out"); // get all files in dist folder
    let promises = [];
    for (const file of files) {
      const data = fs.readFileSync(file);
      const filename = file.replace("./out/", "");
      promises.push(
        fetch(path.join(baseurl, filename), {
          method: "PUT",
          headers: {
            AccessKey: apikey,
  ...
  try {
    const res = await fetch(path.join(pullzone, "/purgeCache"), {
      method: "POST",
      headers: {
